...
add_custom_command(
    ...
    COMMAND ${{LLVM_MLIR_BINARY_DIR}}/mlir-opt ${{BUDDY_EXAMPLES_DIR}}/BuddySST/{subgraph_name}.mlir 
            ...
            ${{LLVM_MLIR_BINARY_DIR}}/llc -filetype=obj  -relocation-model=pic -O0 -o ${{BUDDY_BINARY_DIR}}/../examples/BuddySST/{subgraph_name}.o
    DEPENDS ${{BUDDY_EXAMPLES_DIR}}/BuddySST/{subgraph_name}.mlir
...
add_custom_command(
    OUTPUT {subgraph_name}.o
    COMMAND ${{BUDDY_BINARY_DIR}}/buddy-opt ${{BUDDY_EXAMPLES_DIR}}/BuddySST/{subgraph_name}.mlir 
                -pass-pipeline "builtin.module(func.func(tosa-to-linalg-named, tosa-to-linalg, tosa-to-tensor, tosa-to-arith))" |
            ${{BUDDY_BINARY_DIR}}/buddy-opt
...
add_custom_command(
    OUTPUT forward.o
    COMMAND ${LLVM_MLIR_BINARY_DIR}/mlir-opt ${BUDDY_EXAMPLES_DIR}/BuddySST/forward.mlir 
                -pass-pipeline "builtin.module(func.func(tosa-to-linalg-named, tosa-to-linalg, tosa-to-tensor, tosa-to-arith))" | 
            ...
            ${LLVM_MLIR_BINARY_DIR}/mlir-translate -mlir-to-llvmir |
            ${LLVM_MLIR_BINARY_DIR}/llvm-as |
            ${LLVM_MLIR_BINARY_DIR}/llc -filetype=obj  -relocation-model=pic -O0 -o ${BUDDY_BINARY_DIR}/../examples/BuddySST/forward.o
    DEPENDS ${BUDDY_EXAMPLES_DIR}/BuddySST/forward.mlir
...
add_executable(buddy-sst-run buddy-sst-main.cpp)
target_link_directories(buddy-sst-run PRIVATE ${{LLVM_MLIR_LIBRARY_DIR}})
set(BUDDY_LENET_LIBS WAFER_LENET mlir_c_runner_utils wafer_sst_runtime ${{OpenCV_LIBS}})
target_link_libraries(buddy-sst-run ${{BUDDY_LENET_LIBS}})
    """
    return command

def generate_cmake(driver: GraphDriver):
    generate_set_valuable
    cmake_content = ""
    for subgraph in driver.subgraphs:
        if subgraph.device == DeviceType.GPU:
